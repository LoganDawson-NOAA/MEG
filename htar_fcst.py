...
DIR = os.getcwd() 
DIR = '/lfs/h2/emc/vpppg/noscrub/logan.dawson/MEG'
DATA_HEAD = os.path.join(DIR, 'data')
if not os.path.exists(DATA_HEAD):
   os.makedirs(DATA_HEAD)
...
if cycle is None:
   cycle = input('Enter model initialization time (YYYYMMDDHH): ')
...
if str.upper(model_str) == 'GFS':
   ...
   elif (date_str > gfs_changedate2) and (date_str <= nco_changedate):
   ...
   FILE_PREFIX2 = 'gfs.t'+cycle[8:10]+'z.'
...
# GEFS forecasts
   ...
   MODEL_DIR = 'gefs.'+cycle[0:8]
   runlength = 384
   TAR_PREFIX = 'com2_gens_prod_gefs.'

   if len(model_str) == 4:
      ens_prod = input('Enter MEAN, SPREAD, CTRL, or MEMS for desired GEFS product file: ')
   ...
   grid_res = float(input('Enter 0.5 or 1 for desired grid resolution: '))
   if grid_res == 0.5:
...
# NAM forecasts
   ...
   MODEL_DIR = 'nam.'+cycle[0:8]
   runlength = 84

   nam_edate = datetime.datetime(2016,7,20,0,0,0)

   if date_str < nam_edate or date_str >= nco_changedate:
      TAR_PREFIX = 'com_nam_prod_nam.'
   elif date_str > nam_edate:
      TAR_PREFIX = 'com2_nam_prod_nam.'


   grid_res = input('Enter 12, 32, phys, 3d, or native for desired grid resolution: ')
   if grid_res == '12':
      TAR_SUFFIX   = '.awip.tar'
...
# HREF forecasts
   ...
   MODEL_DIR = 'href.'+cycle[0:8]
   runlength = 36
   
   if date_str >= nco_changedate:
       TAR_PREFIX = 'com_hiresw_prod_href.'
   else:
   ...
   if len(model_str) == 8:
      ...
      ens_prod = input('Enter PROB, MEAN, PMMN, or AVRG for desired HREF product file: ')
      model_str = model_str+ens_prod
   ...
   MODEL_DIR = 'hiresw.'+cycle[0:8]
   runlength = 60
   TAR_PREFIX = 'com_hiresw_prod_hiresw.'
   TAR_SUFFIX = '.5km.tar'

   if len(model_str) == 6:
      hiresw_member = input('Enter ARW, ARW2, or NMMB for desired HiResW run: ')
   ...
   if str.upper(model_str[6:]) == 'NMMB':
      ...
      FILE_SUFFIX  = '.conus.grib2'
   elif str.upper(model_str) == 'CONUSFV3':
      TAR_PREFIX = 'fv3hrw.'
   ...
   if str.upper(model_str) == 'RAP':
      ...
      MODEL_DIR = 'hrrr.'+cycle[0:8]+'/conus'
      if date_str < raphrrr_changedate:
         TAR_PREFIX = 'com2_hrrr_prod_hrrr.'
         runlength = 18
      elif date_str > raphrrr_changedate and date_str < nco_changedate:
...
#===========================================================================================================
if str.upper(model_str) == 'GFS' or str.upper(model_str) == 'NAM' or str.upper(model_str) == 'SREF' or str.upper(model_str) == 'HRRR':
   RH_PREFIX = '/NCEPPROD/hpssprod/runhistory/rh'+cycle[0:4]+'/'+cycle[0:6]+'/'+cycle[0:8]
...
DATA_DIR = os.path.join(DATA_HEAD,MODEL_DIR)
...
for nf in range(len(date_list)):
   ...
   # GFS
   if str.upper(model_str) == 'GFS':
      mv_to = './'+FILE_PREFIX+FILE_SUFFIX+str(fhrs[nf]).zfill(3)

      if date_str >= nco_changedate:
          htar_ball = RH_PREFIX+'/'+TAR_PREFIX+YYYYMMDD_CC+TAR_SUFFIX
          htar_fname  = './gfs.'+cycle[0:8]+'/'+cycle[8:10]+'/atmos/gfs.t'+cycle[8:10]+'z.pgrb2.0p25.f'+str(fhrs[nf]).zfill(3)
          mv_from  = 'gfs.'+cycle[0:8]+'/'+cycle[8:10]+'/atmos/gfs.t'+cycle[8:10]+'z.pgrb2.0p25.f'+str(fhrs[nf]).zfill(3)
          print(htar_ball)
          print(htar_fname)
          print(mv_to)
          print(mv_from)
...
#         htar_fname = './'+FILE_PREFIX+FILE_SUFFIX+str(fhrs[nf]).zfill(3)+' ./'+FILE_PREFIX2+FILE_SUFFIX+str(fhrs[nf]).zfill(3) 
          htar_fname = './'+FILE_PREFIX+FILE_SUFFIX+str(fhrs[nf]).zfill(3) 
