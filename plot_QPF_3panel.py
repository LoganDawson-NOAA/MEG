...
#for j in range(1):
   ...
   fig = plt.figure(figsize=(13,3.75))
   ...
   for ax in axes:
    ...
    # domain = domains[0]  # OK
      ...
      if k == 0:
         model_str = 'HREFAVRG'
         cycle = '2019052000'
         fhrs = np.arange(15,37,3)

         for fhr in fhrs:
             fil = DATA_DIR+'/'+str.lower(model_str[0:4])+'.'+cycle[0:8]+'.t'+ \
                   cycle[8:10]+'z.conus.'+str.lower(model_str[4:])+'.f'+str(fhr).zfill(2)+'.grib2'
      ...
      # Accumulated precip
         ...
         elif str.upper(model_str) == 'HREFAVRG':
             fill_var = np.sum(np.array(APCP),axis=0)
         elif model_str == 'Stage IV':
             fill_var = APCP[0]
      ...
      elif str.lower(model_str[4:]) == 'avrg':
          mod_str = cycle[8:10]+'Z 5/20 '+str.upper(model_str[0:4])+' Average (F'+str(fhrs[-1])+')'
      else:
 ...
 # titlestr = '00Z 5/20 '+field_title+' Forecasts and Analysis '+valid_list[j].strftime('Valid at %HZ %d %b %Y')+' (F'+str(fhrs[j]).zfill(2)+')'
   titlestr = '24-h HREF Average QPFs and Stage IV QPE '+valid_list[j].strftime('Valid at %HZ %d %b %Y')
