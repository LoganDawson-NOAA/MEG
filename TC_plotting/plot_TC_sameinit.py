...
def speedtrace():
 ...
 # plt.plot(plot_ospeed, '-', color='#696969', label='BEST', linewidth=2., marker='D', markersize=7, markevery=markers_on)
   ...
   plt.axis([0,xlen,0,30])
   plt.axhspan(0, 30, facecolor='0.5', alpha=0.5)
...
def plot_lows(plot):
   ...
   for i in range(len(plot_mlats)):
      ...
      if str.upper(mnames[i][0:2]) == 'UK' and fhr <= 144 and fhr%12 != 6:
         plot_ind = int(fhr/12) 
         x, y = m(plot_mlons[i][plot_ind],plot_mlats[i][plot_ind])
    #    elif fhr > 144 or fhr%12 == 6:
    #       sys.exc_clear() 
    # elif str.upper(mnames[i]) != 'UK' and (fhr > 144 or fhr%12 == 6): 
      elif str.upper(mnames[i][0:2]) != 'UK': 
         plot_ind = int(fhr/6)
         try:
             x, y = m(plot_mlons[i][plot_ind],plot_mlats[i][plot_ind])
             ...
             if str.upper(mnames[i][-4:]) == 'MEAN':
                 ax.text(x,y,'L',fontsize=26,fontweight='bold',family='serif',ha='center',va='center',color=line_colors[i],zorder=10)
             elif str.upper(mnames[i][0:2]) == 'UK' and fhr <= 144 and fhr%12 != 6:
                 ax.text(x,y,'L',fontsize=26,fontweight='bold',ha='center',va='center',color=line_colors[i],zorder=10)
             elif str.upper(mnames[i][0:2]) != 'UK':
                 ax.text(x,y,'L',fontsize=26,fontweight='bold',ha='center',va='center',color=line_colors[i],zorder=10)
         except:
