...
fields = ['t2_10mwind','dswrf']
fields = ['dswrf']
if str.upper(model_str) == 'RAP':
    fields = ['hpbl']
elif str.upper(model_str) == 'URMA':
    fields = ['t2','t2_10mwind']
    fields = ['t2_10mwind']
...
#     ncepy.plt_highs_and_lows(m,mslp,lons,lats,mode='reflect',window=HLwindow)
  ...
  #   m.barbs(lons[::skip,::skip],lats[::skip,::skip],u_var[::skip,::skip],v_var[::skip,::skip],latlon=True,length=barb_length,sizes={'spacing':0.2},pivot='middle')
   ...
   # Radiative Flux
      ...
      tlevs = [str(clev) for clev in clevs]
      colormap = plt.get_cmap(name='Spectral_r')
      norm = matplotlib.colors.BoundaryNorm(clevs,colormap.N)

      if field == 'dswrf':
         fill_var = dswrf
         field_title = 'Sfc Downward SW Radiative Flux'

      fill = ax.contourf(lons,lats,fill_var,clevs,cmap=colormap,norm=norm,transform=transform,extend='max')

    # cbar = plt.colorbar(fill,ax=ax,ticks=clevs,orientation='horizontal',pad=0.04,shrink=0.5,aspect=15)
      cbar = plt.colorbar(fill,ax=ax,ticks=clevs,orientation='horizontal',pad=0.01,shrink=0.75,aspect=15)
      cbar.ax.set_xticklabels(tlevs)
      cbar.ax.tick_params(labelsize=8)
...
#        precip = precip + bucket
   ...
   # RAP
      ...
      #  grbs = pygrib.open(fil)
         ...
         fil = DATA_DIR+'/'+str.lower(model_str)+'.'+cycle[0:8]+'/'+ \
               str.lower(model_str)+'.t'+cycle[8:10]+'z.awp130pgrbf'+str(fhrs[j]).zfill(2)+'.grib2'
         grbs = grib2io.open(fil,mode='r')
...
#     precip = grbs.select(name='Total Precipitation')[0].values/25.4    # first precip message is run-total bucket; second is 1-h bucket
   ...
   # URMA
      ...
      try:
         fil = '/lfs/h1/ops/prod/com/urma/v2.10/urma2p5.20230501/'+os.environ['gfs_ver']+'/gfs.'+cycle[0:8]+'/'+cycle[8:10]+'/atmos/'+ \
               str.lower(model_str)+'.t'+cycle[8:10]+'z.pgrb2.0p25.f'+str(fhrs[j]).zfill(3)
         grbs = grib2io.open(fil,mode='r')
      except:
         fil = DATA_DIR+'/'+str.lower(model_str)+'2p5.'+cycle[0:8]+'/'+ \
               str.lower(model_str)+'2p5.t'+cycle[8:10]+'z.2dvaranl_ndfd.grb2_wexp'
         grbs = grib2io.open(fil,mode='r')
  ...
  #   aotk = grbs.select(shortName='AOTK')[0].data
   ...
   if 't2' in fields or 't2_10mwind' in fields:
       t2 = grbs.select(shortName='TMP',level='2 m above ground')[0].data*9/5 - 459.67

   if 'td2' in fields or 'td2_10mwind' in fields:
       td2 = grbs.select(shortName='DPT',level='2 m above ground')[0].data*9/5 - 459.67

   if 't2_10mwind' in fields or 'td2_10mwind' in fields or '10mwind' in fields: 
   ...
   if 'dswrf' in fields:
       dswrf = grbs.select(shortName='DSWRF')[0].data
...
#     grb = grbs.select(shortName='TMP',level='2 m above ground')[0]
   ...
   if str.upper(model_str[0:4]) != 'HREF' or str.upper(model_str) != 'AQM' or str.upper(model_str) != 'URMA':
